  ...
  return (
    <div className='flex flex-col items-center w-full max-w-3xl text-sm '>
        <div className={`flex flex-col w-full mt- 8 ${role === 'user' && 'items-end '}`}>
            <div className={`group relative flex max-w-2xl py-3 rounded-lg ${role === 'user' ? 'bg-[#414158] px-5':'gap-3'}`}>
                ...
                </div>
                {
                    ...
                    (
                        <>
                            <Image src={assets.logo_icon} alt='' className='h-9     w-9 p-1 boder border-white/15 rounded-full'/>
                            <div className='space-y-4 overflow-scroll w-full'>{content} </div>
                        </>
                    )
                }
            </div>
